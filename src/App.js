...
export default function App() {
  function reducer(state, action) {
    ...
    switch (action.type) {
      ...
      case "pay-loan":
        return{
          ...state, 
          loan : state.loan > 0 && state.balance >= state.loan ? state.loan - 5000 : state.loan,
  ...
  return (
    <div className="App">
      ...
      <p>
        <button onClick={() => {dispatch({type : "request-loan5000"})}} disabled={!isActive}>
          Request a loan of 5000
        </button>
      </p>
      <p>
        <button onClick={() => {dispatch({type : "pay-loan"})}} disabled={!isActive}>
          Pay loan
        </button>
      </p>
      <p>
        <button onClick={() => {loan === 0 && balance === 0 && dispatch({type : "close-account"})}} disabled={!isActive}>
          Close account
        </button>
      </p>
